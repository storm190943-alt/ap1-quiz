...
let idx = 0;
let locked = false;
let multiStage = 0; // 0=selecting, 1=evaluated (learn mode)
...
function renderQuestion() {
  ...
  els.nextBtn.textContent = (q.type === 'multi' && els.mode.value === 'learn') ? 'Prüfen' : 'Weiter';
  ...
  q.answers.forEach(a => {
    ...
    if (q.type === 'multi') {
      btn.onclick = () => {
        if (locked) return;
        if (multiSelected.has(a)) {
          multiSelected.delete(a);
          btn.classList.remove('sel');
        } else {
          multiSelected.add(a);
          btn.classList.add('sel');
        }
        // enable 'Prüfen/Weiter' once at least one option is selected
        els.nextBtn.disabled = (multiSelected.size === 0);
      };
    } else {
...
function choose(answer, btn) {
  ...
  if (els.mode.value === "learn") {
    ...
    for (const b of buttons) {
      if (correctSet.has(b.textContent)) b.classList.add('ok');
    }
  } else {
...
function evaluateMulti() {
  ...
  // In exam mode we just store and move on
  if (els.mode.value === "exam") {
    const chosen = [...multiSelected];
    // keep only one record per qid
    examAnswers = examAnswers.filter(x => x.qid !== q.id);
    examAnswers.push({ qid: q.id, chosen, correct: q.correct, topicId: q.topicId, topicTitle: q.topicTitle, type: "multi" });
    answered += 1;
    els.feedback.textContent = "Antwort gespeichert.";
    return;
  }

  // learn mode: first click on next is "Prüfen"
  if (!locked && multiStage === 0) {
    locked = true;
    multiStage = 1;
    ...
    let isCorrect = chosen.length === q.correct.length;
    if (isCorrect) {
      for (const c of q.correct) {
        if (!chosenSet.has(c)) { isCorrect = false; break; }
      }
    }
    ...
    for (const b of buttons) {
      ...
      const isRight = correctSet.has(a);
      const isChosen = chosenSet.has(a);
      b.disabled = true;
      if (isRight) b.classList.add("ok");
      if (isChosen && !isRight) b.classList.add("bad");
    }
    ...
    els.score.textContent = String(score);
    els.nextBtn.textContent = "Weiter";
    els.nextBtn.disabled = false;
  }
}
...
function next() {
  const q = run[idx];
  if (q.type === 'multi') {
    // In learn mode: if not evaluated yet, evaluate first
    if (els.mode.value === 'learn' && !locked) {
      evaluateMulti();
      return;
    }
    // In exam mode: store chosen (if any) before moving on
    if (els.mode.value === 'exam') {
      if (multiSelected.size > 0) evaluateMulti();
    }
  }
  if (idx < run.length - 1) {
    idx += 1;
